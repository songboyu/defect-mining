...
class Fuzzer(object):
    ...
    def __init__(self, binary_path, work_dir, afl_count=1, library_path=None, time_limit=None,
        ...
        if self.is_multicb:
            ...
            # XXX: There is no concolic/angr support, and probably will never be.
            self.afl_path = afl_wrapper.afl_bin('multi-cgc')
            self.afl_path_var = afl_wrapper.afl_path_var('multi-cgc')
        else:
            ...
            self.afl_dir          = afl_wrapper.afl_dir(self.os)

            # the path to AFL capable of calling concolic
            self.afl_path         = afl_wrapper.afl_bin(self.os)

            if self.os == 'cgc':
                self.afl_path_var = afl_wrapper.afl_path_var('cgc')
            else:
                self.afl_path_var = afl_wrapper.afl_path_var(p.arch.qemu_name)
                # set up libraries
    ...
    def _start_afl(self):
        ...
        if self.afl_count > 1:
            concolic = self._start_afl_instance()
            self.procs.append(concolic)
