...
l = logging.getLogger("mining.concolic")
l.setLevel("DEBUG")

class ConcolicEnvironmentError(Exception):
    pass

class ConcolicMisfollowError(Exception):
    pass

class Concolic(object):
    '''
    ...
    def __init__(self, binary, input, fuzz_bitmap='\xff'*65535, tag=None, redis=None, hooks=None):
        ...
        # setup directories for the concolic and perform sanity checks on the directory structure here
        if not self._sane():
            l.error("[%s] environment or parameters are unfit for a concolic run", self.identifier)
            raise ConcolicEnvironmentError
